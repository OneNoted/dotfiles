...
For dev-tool homes that would otherwise spill into top-level dotdirs, the repo exports XDG-aware tool homes for Cargo, Rustup, Bun, Gradle, npm, Dart pub, Docker, and safe cache/tool state for Go, Python tooling, Node tooling, K9s/Helm, Mise, and Starship. Use `xdg-dev-home-migrate --plan` to preview the supported destructive moves and `xdg-dev-home-migrate --migrate` to relocate the existing directories into the configured XDG targets with dated backups under `~/.local/state/`.
