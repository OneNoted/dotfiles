...
function __paru_listall
    set -l tok (commandline -ct)
    set tok (string trim -- $tok)

    if test -z "$tok"; or test (string length -- $tok) -lt 2
        return
    end

    paru -Ssq --color always -- "$tok" 2>/dev/null | string replace ' ' \t
end
set -l listall "(__paru_listall)"
# set -l listall "(paru -Pc | string replace ' ' \t)" # broken until https://github.com/Morganamilo/paru/issues/1447 gets resolved. 
